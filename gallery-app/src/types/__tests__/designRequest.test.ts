...
describe('design request snapshots', () => {
  it('creates an immutable versioned copy with exact design values', () => {
    ...
    const source = {
      ...
      shape: { height: 40, bodyWidth: 32, neckWidth: 12, rimSize: 14, curvature: 72, unit: 'in' as const },
      ...
      attachments,
      dimensions: { heightIn: 40, widthIn: 32, unit: 'in' as const }, estimate: { price: 1550, productionDays: 7 },
    };
  ...
  it('normalizes unknown finishes and rejects malformed message envelopes', () => {
    const snapshot = createDesignRequestSnapshot({
      ...
      shape: { height: 25, bodyWidth: 20, neckWidth: 15, rimSize: 12, curvature: 50, unit: 'in' as const },
      ...
      dimensions: { heightIn: 25, widthIn: 20, unit: 'in' as const }, estimate: { price: 1250, productionDays: 5 },
    });
  ...
  it('converts legacy centimeter request snapshots without changing the design shape', () => {
    const snapshot = normalizeDesignRequestSnapshot({
      version: 1,
      model: { id: null, name: 'Legacy pot', file: '/pot.glb', thumbnail: '', category: 'Pot' },
      shape: { height: 20, bodyWidth: 10, neckWidth: 5, rimSize: 4, curvature: 50 },
      material: { finish: 'raw_clay', color: '#BE734F' },
      decoration: { patternId: '', placement: 'middle', scale: 1, color: '#7A3E12', effect: 'painted' },
      attachments: [],
      dimensions: { heightCm: 20, widthCm: 10 },
