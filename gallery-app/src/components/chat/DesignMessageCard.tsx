...
import { useQuoteReview } from './QuoteReviewContext';
  ...
  if (requestPayload) {
    ...
    return <div className="chat-design-request-card">
      ...
      {loading ? <div className="chat-design-request-loading"><LoaderCircle className="seller-spin" /> Loading request…</div> : request && summary ? <>
        ...
        {audience === 'buyer' && request.status === 'quoted' ? <button type="button" className="chat-design-approve" onClick={() => openQuoteReview(request.id)}><CheckCircle2 /> Review Shop Quote</button> : null}
