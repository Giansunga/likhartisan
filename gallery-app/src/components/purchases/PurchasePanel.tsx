...
export default function PurchasePanel({ reviewedProductIds = EMPTY_REVIEWED_PRODUCTS, onRate }: Props) {
  ...
  async function pay(order: PurchaseSummary) { if (!order.checkoutSessionId) return toast.error('No payment session found.'); try { const response = await fetch(`${API_BASE}/api/session/${order.checkoutSessionId}`); const session = await response.json(); if (session.checkout_url) { localStorage.setItem('likhartisan_checkout_session_id', order.checkoutSessionId); window.location.assign(session.checkout_url); } else toast.info('This payment session has expired.'); } catch { toast.error('Payment is temporarily unavailable.'); } }
