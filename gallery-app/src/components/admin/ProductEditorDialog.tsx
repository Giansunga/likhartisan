...
export default function ProductEditorDialog({ product, onClose, onSave }: ProductEditorDialogProps) {
  ...
  useEffect(() => {
    ...
    void supabase.from('product_variations').select('*').eq('product_id', product.id).order('sort_order').then(({ data, error }) => {
      ...
      else setVariations((data ?? []).map((item: any) => ({ id: item.id, dimensions: item.dimensions ?? '', height: item.height ?? '', openingDiameter: item.opening_diameter ?? '', weightKg: item.weight_kg == null ? '' : String(item.weight_kg), price: item.price == null ? '' : String(item.price), stock: String(item.stock ?? 0) })));
