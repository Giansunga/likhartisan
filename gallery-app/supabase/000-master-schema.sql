...
CREATE TABLE IF NOT EXISTS models_3d (
  ...
  base_height_in NUMERIC(5,2) CHECK (base_height_in BETWEEN 2 AND 20),
  base_body_width_in NUMERIC(5,2) CHECK (base_body_width_in BETWEEN 2 AND 16),
  base_neck_width_in NUMERIC(5,2) CHECK (base_neck_width_in BETWEEN 1 AND 12),
  base_rim_size_in NUMERIC(5,2) CHECK (base_rim_size_in BETWEEN 1 AND 10),
  base_price_php NUMERIC(12,2) CHECK (base_price_php > 0),
  base_production_days INTEGER CHECK (base_production_days BETWEEN 1 AND 365),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
