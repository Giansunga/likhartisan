...
-- Run in Supabase SQL Editor (safe: drops and recreates all policies)
-- =============================================================================
-- Roles:
--   Admin      = giansunga396@gmail.com
--   Shop owner = shops.owner_id = auth.uid()
--   Buyer      = orders.user_id / conversations.buyer_id = auth.uid()
--   Artisan    = anyone (display-only, no auth needed)
-- =============================================================================
...
-- =============================================================================
-- SHOPS
-- Public read. Admin-only write. Owner can update own shop.
-- =============================================================================
...
CREATE POLICY "shops_update_admin_or_owner" ON public.shops
...
-- =============================================================================
-- PRODUCTS
-- Public read. Admin or shop owner write.
-- =============================================================================
...
CREATE POLICY "products_insert_admin_or_owner" ON public.products
  FOR INSERT WITH CHECK (
    public.is_admin() OR public.is_shop_owner(shop_id)
  );
...
CREATE POLICY "products_delete_admin_or_owner" ON public.products
  FOR DELETE USING (
    public.is_admin() OR public.is_shop_owner(shop_id)
  );
...
-- =============================================================================
-- PRODUCT VARIATIONS
-- Public read. Shop owner of parent product manages.
-- =============================================================================
...
CREATE POLICY "variations_manage_owner" ON public.product_variations
  FOR ALL USING (
    ...
    OR public.is_shop_owner(
      (SELECT shop_id FROM public.products WHERE id = product_id)
    )
  );

-- =============================================================================
-- ORDERS
-- Buyers see own orders. Shop owners see orders containing their products.
-- Buyers insert own orders. Buyers can cancel unpaid orders.
-- Artisans update delivery_status on shop orders.
-- =============================================================================
...
-- =============================================================================
-- CONVERSATIONS
-- Buyer sees own. Shop owner sees own shop's conversations.
-- =============================================================================
...
-- =============================================================================
-- MESSAGES
-- Participants of parent conversation can read/send.
  ...
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE c.id = conversation_id
        AND (
          c.buyer_id = auth.uid()
          OR public.is_shop_owner(c.shop_id)
          OR public.is_admin()
        )
    )
  );
...
CREATE POLICY "models_select_public" ON public.models_3d
  FOR SELECT USING (true);

CREATE POLICY "models_insert_admin" ON public.models_3d
  FOR INSERT WITH CHECK (public.is_admin());

CREATE POLICY "models_update_admin" ON public.models_3d
  FOR UPDATE USING (public.is_admin());

CREATE POLICY "models_delete_admin" ON public.models_3d
  FOR DELETE USING (public.is_admin());
...
CREATE POLICY "theme_insert_admin" ON public.theme_settings
  FOR INSERT WITH CHECK (public.is_admin());

CREATE POLICY "theme_update_admin" ON public.theme_settings
  FOR UPDATE USING (public.is_admin());

-- =============================================================================
-- SHOP_FOLLOWERS
-- Public read. Users manage own follows.
-- =============================================================================
...
CREATE POLICY "followers_select_public" ON public.shop_followers
  FOR SELECT USING (true);

CREATE POLICY "followers_insert_own" ON public.shop_followers
  FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "followers_delete_own" ON public.shop_followers
  FOR DELETE USING (user_id = auth.uid());

-- =============================================================================
-- PRODUCT_REVIEWS (already well-scoped, just clean up naming)
-- =============================================================================
...
-- =============================================================================
-- DESIGNS (already well-scoped, just clean up naming)
-- =============================================================================
...
-- =============================================================================
-- NOTIFICATIONS (already well-scoped, just clean up naming)
-- =============================================================================
...
-- =============================================================================
-- STORAGE — restrict uploads/deletes to authenticated users
-- =============================================================================
